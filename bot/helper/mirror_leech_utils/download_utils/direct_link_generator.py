...
def terabox(link: str) -> str:
    try:
        # Let requests encode the query param properly
        r = requests.get(
            "https://api.throwbin.in/website.php",
            params={"link": link},
            timeout=10,
        )
        r.raise_for_status()  # raise HTTPError on 4xx/5xx [web:13]
        data = r.json()       # decode JSON body [web:13]

        # Expecting keys like: fileName, fileSize, uploadedAt, downloadLink, fastDownloadLink
        if isinstance(data, dict) and data.get("downloadLink"):
            return data["downloadLink"]  # only return the direct link [web:13]
        # Some endpoints occasionally wrap in a list; handle defensively
        if isinstance(data, list) and data and isinstance(data[0], dict) and data[0].get("downloadLink"):
            return data[0]["downloadLink"]  # return the first direct link [web:13]

        raise DirectDownloadLinkException("downloadLink not found in API response")
    except requests.exceptions.RequestException as e:
        # Network, timeout, or HTTP error (after raise_for_status)
        raise DirectDownloadLinkException(f"Failed to fetch Terabox link: {e}") from e
    except ValueError as e:
        # JSON parse error
        raise DirectDownloadLinkException(f"Invalid JSON from API: {e}") from e
